...
class GildedRoseTest < Minitest::Test
  def update_with(sell_in:, quality:, name:, sell_in_change: -1, quality_change: -1)
    ...
    expected_sell_in = sell_in + sell_in_change
    expected_quality = quality + quality_change

    assert_equal expected_sell_in, item.sell_in, "Sell in was #{item.sell_in} and should be #{expected_sell_in}"
    assert_equal expected_quality, item.quality, "Quality was #{item.quality} and should be #{expected_quality}"
  end
